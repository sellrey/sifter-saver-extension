...
- Job types: `sift-only`, `scan-and-sift`, `scan-only`. Title `h2.job-config__title` is "Sift job setup" / "Scan and sift job setup" / "Scan job setup".
...
- Per-game rules: YGO hides color/foil/rarity; ONE hides color. PKM calls color "Energy" (card) / "Energy type" (select); LOR "Ink type". Price sift allows NM/LP/MP for MTG and PKM, NM only otherwise. Option tables live in `src/catalog.js`.
